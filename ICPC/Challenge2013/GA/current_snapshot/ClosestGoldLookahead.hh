  ...
  */
  void computeEnemies() {
	...
	for (int y = 0; y < HEIGHT; ++y) {
	  ...
	  Board &origBoard = se->ourStatus->board;
	  ...
	  if (valid_indices(y+1,x) && last[x][y+1] < i-1 &&
	      origBoard.visible(y+1,x) == LADDER)
...
private:
  ...
  void init() {
    ...
    for (int i = 0; i < (int)enemies.size(); ++i)
      ...
      if (valid_indices(enemies[i].pos.row, enemies[i].pos.col) &&
	  board(enemies[i].pos.row, enemies[i].pos.col) != REMOVED_BRICK)
	lookahead_[0][(uchar)enemies[i].pos.col][(uchar)enemies[i].pos.row] = -1;
  }
};
...
vector<pii> RoutesAvailable(int x, int y) {
  ...
  for (int i = 0; i < (int)golds.size(); ++i) {
    ...
    if (dist != oo && dist > 0 && 
	dist >= board.gold_respawns_in(golds[i].row,golds[i].col))
      res.push_back(dest);
  }
  return res;
}
...
private:
  ...
  double pathWeight(int x, int y, pii dest) {
    double weight = 1.0;
    for (int i = 1; i <= min(TURNS_REMAINING, N_LOOKAHEAD_STEPS); ++i) {
	...
	for (int j = 0; j < (int)enemies.size(); ++j)
	  if (enemies[j].pos.row == y+1 && enemies[j].pos.col == x) {
	    blocked = true; break;
	  }
	if (!blocked) {
	  int ey;
