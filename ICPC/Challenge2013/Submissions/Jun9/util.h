...
pair<cmd,int> suicide_move(){
  ...
  if(!enemies2.empty()){
    ...
    if(!v.empty()){
      // They walk towards you a bit...
      m1 = max(0,distance(P1.row,P1.col,enemies2[0].row,enemies2[0].col) - 2);
      c1 = v[0];
    }    
  }
  ...
  if(board(P1.row,P1.col) == REMOVED_BRICK) return make_pair(NONE,25);
  
  //  Method 2(b): Is the hole already dug?
  if(board(P1.row+1,P1.col-1) == REMOVED_BRICK) return make_pair(LEFT,27);
  if(board(P1.row+1,P1.col+1) == REMOVED_BRICK) return make_pair(RIGHT,27);
