...
vd kill_yourself(const vector<bool>& moves){
   ...
   // Do not kill yourself the first 25 seconds after you respawn...
   if(alive_time <= 25) return ans;
...
vd kill_opp(const vector<bool> &moves) {
  ...
  int r,c;
  curr_loc(r,c);
  if (distance(r, c, P2.row, P2.col) == 0) {
    ans = vd(7,-1);
    ans[DIG_LEFT] = ans[DIG_RIGHT] = -1;
    ans[NONE] = 0;
  } else if (distance(r, c, P2.row, P2.col) <= 3) {
    vector<cmd> v = Goto(r, c, P2.row, P2.col);
    for (size_t i = 0; i < v.size(); i++) {
      if (v[i] == LEFT) {
	ans[DIG_LEFT] = 1;
...
vd never_run_into_opp_hole(const vector<bool>& moves){
  ...
  for(int i=0;i<7;i++){
    end_up(cmd(i),r,c);
    if(board(r,c) == REMOVED_BRICK && opp_dug_me[r][c]){
      found = true;
...
vd no_walk_into_filled_removed_brick(const vector<bool>& moves){
  ...
  if(moves[LEFT] && board(P1.row,P1.col-1) == REMOVED_BRICK){
    for(size_t i=0;i<enemies.size();i++){
      if((enemies[i].row == P1.row   && enemies[i].col == P1.col-1) ||
	 (enemies[i].row == P1.row-1 && enemies[i].col == P1.col-1))
	ans[LEFT] = -1;
    }
  }
...
vd dont_go_into_leaf_scc(const vector<bool> &moves){
  ...
  for (int i = 0; i < 7; i++) {
    ...
    if (!origBoard.is_SCC_leaf(r, c)) continue;
    ans[i] = -1 + 0.05 * golds_in_scc[origBoard.scc_index(r, c)];
    ans[i] = min(ans[i], 1.0);
  }
  return ans;
}
