...
//
//  TODO Heuristics
//    - If enemy detected, dig/run
//    - If Player 2 detected, dig/run
//    - 
//
// *****************************************************************************
// *****************************************************************************
...
vd visit_unvisited(const vector<bool>& moves){
  ...
  for(int k=0;k<7;k++){
    ...
    if(closest - 1 == currBoard.dist_with_fall[nI][nJ][best_r][best_c])
      ans[k] = val;
  }
...
vd kill_yourself(const vector<bool>& moves){
   ...
   if(!enemies.empty())
      ...
      disten = threshold;
   if(distgold >=threshold && disten >= threshold){
      ans = avoid_falling_death(moves);
      for(int i=0;i<7;i++)
	 ans[i] = -ans[i];
      return ans;
   }
