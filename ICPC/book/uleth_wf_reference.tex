...
\begin{center}
 {\Huge\bf{ACM ICPC World Finals 2014 Code Booklet}}\\\vskip5pt
...
\section{Number Theory}
  ...
  \code{eulerphi.cc}{Euler Phi Function}
  \code{expmod.cc}{Exponentiation Mod m}
  \code{euclid.cc}{Extended Euclidean Algorithm}
...
\section{Dynamic Programming}
  \code{asc_subseq.cc}{Longest Increasing Subsequence}
...
\section[Graph Theory ~~ (Directed\directed ~~ or ~~ Undirected\undirected ~~ or  ~~ Both\both)]{Graph Theory}
  ...
  \code{bicomp.cc}{Biconnected Components \undirected}
%  \code{dijkstra.cc}{Dijkstra's Algorithm for Dense Graphs \both}
  \code{dijkstra_sparse.cc}{Dijkstra's Algorithm for Sparse Graphs \both}
  \code{eulertour.cc}{Eulerian Tour \both}
  \code{floyd_path.cc}{Floyd's Algorithm with Path finding \both}
  \code{hungarian.cc}{Hungarian Algorithm \directed}
  \code{lca.cc}{Least Common Ancestor \directed}
  \code{matching.cc}{Bipartite Matching \undirected}
  \code{genmatching.cc}{General Graph Matching \undirected}
  \code{mincostmaxflowdense.cc}{Min-Cost Max-Flow for Dense Graphs \both}
  \code{mincostmaxflowsparse.cc}{Min-Cost Max-Flow for Sparse Graphs \both}
  \code{min_cut.cc}{Minimum Cut of a Graph \undirected}
  \code{mst.cc}{Minimum Spanning Tree \undirected}
  \code{networkflow.cc}{Network Flow $O(fm)$ \both}
  \code{networkflow2.cc}{Network Flow $O(n^3)$ \both}
  \code{scc.cc}{Strongly Connected Components \directed}
  \code{2sat.cc}{2-SAT \directed}
  \code{top_sort.cc}{Topological Sort \directed}
  \code{tree_distance.cc}{Tree Distances \undirected}

\section{Linear Algebra}
  \code{diophantine_sys.c}{System of Linear Diophantine Equations}
  \code{fflinsolve.cc}{Fraction Free Linear Solve}
  \code{linsolve.cc}{LU-Decomposition}

\section{Data Structures}
%  \code{fenwicktree.cc}{Fenwick Tree}
  \code{fraction.cc}{Fraction Class}
  \code{unionfind.cc}{Union Find}
  \code{date.cc}{Date Class}
  \code{rmq.cc}{Range Maximum Query}
%  \code{min_queue.cc}{Queue with Minimum Maintained}
%  \code{priority_queue.cc}{Priority Queue with Handles}
  \code{upper_hull.cc}{Minimum/Maximum Value of Set of Linear Lines}  
\section{String Processing}
  \code{kmp.cc}{KMP}
%  \code{str_rotation_period.cc}{String Rotation Period}
  \code{suffixarray.cc}{Suffix Array}

\section{Game Theory}
  \code{alpha_beta.cc}{Alpha-Beta Pruning}

\section{Algorithms and Misc}
  \code{cubic.cc}{Cubic Polynomial Solver}
%  \code{de_bruijn.cc}{De Bruijn Sequences}
%  \code{frac2dec.cc}{Fraction to Decimal}
  \code{hash.cc}{Hashing Custom Types}
  \code{infix.cc}{Infix Evaluations}
  \code{josephus.cc}{Josephus Problem}
  \code{poly_mult.cc}{Polynomial Multiplication using FFT}
  \code{roman_numerals.cc}{Conversion to and from Roman Numerals}
  \code{simpsons_rule.cc}{Simpson's Rule}
  \code{rubik.cc}{Rubik's Cube}
