...
// from e-maxx eng slightly modified for code book
...
namespace algebra {
  ...
  template<typename T>
	...
	template<typename T>
	T nCr(T n, int r) {
    T res(1); for (int i = 0; i < r; i++) { res *= (n - T(i)); res /= (i + 1); }
    return res; 
  }
  ...
  struct poly {
    ...
    poly mod_xk(int k) const {
      k = min(k, sz(a)); return vector<T>(begin(a), begin(a) + k); 
    }
    poly mul_xk(int k) const {
      poly res(*this); res.a.insert(begin(res.a), k, 0); return res; 
    }
    poly div_xk(int k) const {
      k = min(k, sz(a)); return vector<T>(begin(a) + k, end(a)); 
    }
    ...
    poly inv(int n) const { // get inverse series mod x^n
      ...
      while (a < n) { 
        ...
        ans -= (ans * C).mod_xk(a).mul_xk(a); a *= 2; 
      }
      return ans.mod_xk(n);
    }
    ...
    poly operator *= (const T &x) { 
      for (auto &it: a) it *= x; normalize(); return *this; 
    }
    poly operator /= (const T &x) { 
      for (auto &it: a) it /= x; normalize(); return *this; 
    }
    ...
    T eval(T x) const { 
      T res(0); 
      for (int i = sz(a) - 1; i >= 0; i--) res *= x, res += a[i];
      return res; 
    }
    ...
    T operator [](int idx) const { 
      return idx >= sz(a) || idx < 0 ? T(0) : a[idx]; 
    }
